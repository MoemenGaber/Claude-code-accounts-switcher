...
const { HISTORY_ARTIFACTS, ALWAYS_STRIPPED } = require('./clear');
const { getOS } = require('./os');
const { detectShellProfile } = require('./shell');
const HISTORY_SET      = new Set(HISTORY_ARTIFACTS);
const ALWAYS_STRIP_SET = new Set(ALWAYS_STRIPPED);
...
async function promptDir(name, existingAccounts) {
  ...
  while (true) {
    ...
    if (!path.isAbsolute(dir)) {
      console.log('  Path must be absolute.'); continue;
    }
...
function cloneConfig(src, dest, { includeHistory = false } = {}) {
  ...
  for (const entry of entries) {
    if (ALWAYS_STRIP_SET.has(entry)) continue;
    if (!includeHistory && HISTORY_SET.has(entry)) continue;
...
async function addAccount(nameArg, dirArg) {
  ...
  const reloadHint = getOS() === 'windows' ? '. $PROFILE' : (profile ? `source ${profile}` : 'source ~/.zshrc');
  console.log(`Run: ${reloadHint}   (to activate the 'claude${name}' alias)`);
}
