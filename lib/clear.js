...
// Conversation data — optionally copied when the user clones with history
...
// Auth/env caches — never copied between accounts regardless of user choice
const ALWAYS_STRIPPED = [
  'session-env',
  'mcp-needs-auth-cache.json',
];

// Union used by clearHistory to wipe everything in one pass
const SESSION_ARTIFACTS = [...HISTORY_ARTIFACTS, ...ALWAYS_STRIPPED];
...
async function clearHistory(nameArg) {
  ...
  if (!name || !accounts.has(name)) {
    // Unknown name: warn but fall through to picker rather than hard-exiting
    if (nameArg) {
...
module.exports = { clearHistory, SESSION_ARTIFACTS, HISTORY_ARTIFACTS, ALWAYS_STRIPPED };
