...
function detectShellProfile() {
  ...
  if (platform === 'windows') {
    // Standard PowerShell 7+ (cross-platform) profile path
...
function global:claude {
  ...
  if ($PassArgs.Count -gt 0 -and (Test-Path $conf) -and (Select-String -Path $conf -Pattern "^$($PassArgs[0])=" -Quiet 2>$null)) {
    & _cca use --silent $PassArgs[0]
    $PassArgs = $PassArgs[1..$PassArgs.Count]
  }
  ...
  if ($name -and (Test-Path $conf)) {
    $line = Get-Content $conf | Where-Object { $_ -match "^$name=" } | Select-Object -First 1
